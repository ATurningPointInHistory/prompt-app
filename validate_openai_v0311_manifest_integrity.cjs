...
const changed=["17_external_intelligence_openai_provider_integration.js","17_external_intelligence_openai_provider_ui.js"];
check("v0.3.11 changed files use refreshed cache keys",changed.every(f=>index.includes(`./${f}?h=${manifest.hashes[f].cacheKey}`)),changed.map(f=>({file:f,cacheKey:manifest.hashes[f].cacheKey})));
const failed=checks.filter(x=>!x.passed);console.log(JSON.stringify({id:"OPENAI-V0.3.11-MANIFEST-INTEGRITY",candidateVersion:"0.3.11",passed:checks.length-failed.length,failed:failed.length,total:checks.length,health:Math.round((checks.length-failed.length)/checks.length*100),criticalFailed:failed.length,scriptCount:manifest.scripts.length,manifestHash:manifest.manifestHash,scriptSetHash:manifest.scriptSetHash,checks},null,2));process.exitCode=failed.length?1:0;
