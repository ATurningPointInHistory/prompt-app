...
(function (global) {
  ...
  function validateRecord(record){const cv=namespace.validateExternalIntelligenceContract("predictionRecord",record);const sv=namespace.validateExternalIntelligenceRecord("EXTERNAL-010-SCHEMA-PREDICTION-RECORD",record);return {valid:cv.valid===true&&sv.valid===true,contract:cv,schema:sv};}
  function registerExternalIntelligencePrediction(input){const x=internal.isPlainObject(input)?input:{}; const predictionId=internal.text(x.predictionId,""); if(!predictionId)return internal.buildResult(false,"EXTERNAL010_PREDICTION_ID_REQUIRED","Blocked",null);
    ...
    const validity=validateRecord(rec); if(!validity.valid)return internal.buildResult(false,"EXTERNAL010_PREDICTION_RECORD_INVALID","Blocked",validity);
