...
function validateContext(config, body, options) {
  ...
  const bodyPolicy=isPlainObject(operation.bodyPolicy)?operation.bodyPolicy:{mode:"NONE",required:[],optional:[],allowUnknown:false,maxSerializedBytes:0};
  ...
  if(operation.method==="GET"){ if(Object.keys(requestBody).length) throw buildError("GET_BODY_NOT_ALLOWED","INVALID_REQUEST",400,false); }
    ...
    if(bodyPolicy.allowUnknown!==true){const allowed=new Set([...(Array.isArray(bodyPolicy.required)?bodyPolicy.required:[]),...(Array.isArray(bodyPolicy.optional)?bodyPolicy.optional:[])]); for(const key of bodyKeys) if(!allowed.has(key)) throw buildError("BODY_FIELD_UNKNOWN","INVALID_REQUEST",400,false,key);}
