...
(function (global) {
  ...
  async function runExternalIntelligencePhase19Validation() {
    ...
    add("Release is Phase 19 v1.18.0 compatible", VM.isReleaseCompatibleFrom("1.18.0"), VM.release.version, "Foundation");
    ...
    const ruleSnapshot = namespace.createExternalIntelligenceMarketRuleSnapshot({ marketRuleSnapshotId: ids.ruleSnapshotId, version: 1, marketRuleProfileId: ids.ruleProfileId, periodStart: "2026-01-01", periodEnd: "2026-09-05", tradingUnit: 1, priceLimit: { type: "VENUE_RULE" }, marketControlRule: { state: "NORMAL" }, historicalEvidenceRefs: ["P19-RULE-EVIDENCE"], historicalRuleIntegrityState: "PASS" });
    ...
    add("Audit and Lineage integrations record Experiment/Backtest/Readiness derivation", auditEvents.some(function e(e) { return e.eventType === "BACKTEST_COMPLETED"; }) && auditEvents.some(function e(e) { return e.eventType === "STRATEGY_READINESS_CREATED"; }) && lineage && Array.isArray(lineage.references) && lineage.references.length > 0, { auditCount: auditEvents.length, lineage }, "Lineage");
