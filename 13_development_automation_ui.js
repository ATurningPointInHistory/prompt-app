...
(function (global) {
  ...
  const viewState = { minimized: false, busy: false, lastReflectionPackageId: null, lastMessage: "" };
...
#${UI_ID} .ide190-body{overflow:auto;padding:12px;display:grid;gap:12px}#${UI_ID} .ide190-section{border:1px solid #374151;border-radius:10px;padding:11px;background:#182131}
...
`;
  ...
  function statusHtml() {
    ...
    return cards.map(function card(item) { return '<div class="ide190-card"><div class="ide190-label">' + escapeHtml(item[0]) + '</div><div class="ide190-value">' + escapeHtml(item[1]) + '</div></div>'; }).join("");
  }
  ...
  function render() {
    ...
    if (message) message.textContent = viewState.lastMessage || "UIは権限を増やしません。ReflectionはX1手動準備のみです。";
    container.classList.toggle("ide190-minimized", viewState.minimized === true);
    container.querySelectorAll("button[data-action]").forEach(function disable(btn) { btn.disabled = viewState.busy; });
  }
...
<div class="ide190-panel">
  <div class="ide190-header"><div class="ide190-title">IDE-190 Development Automation</div><div class="ide190-sub">Phase 9 Safe UI</div><button data-action="minimize">最小化</button><button data-action="close">閉じる</button></div>
  <div class="ide190-body">
    ...
    <section class="ide190-section"><strong>Reflection Package（X1 / 手動反映準備）</strong><p class="ide190-muted">対象ファイルは推測しません。1行1ファイルで明示してください。準備だけではDownloadやGitHub反映は行いません。</p><textarea data-role="files" placeholder="例:\n13_development_automation_ui.js\n13_development_automation_reflection.js"></textarea><div style="height:7px"></div><input data-role="receipt" placeholder="Automation Receipt ID（任意）"><div class="ide190-row" style="margin-top:8px"><button class="primary" data-action="prepare">Reflection Package準備</button><button class="success" data-action="download">ZIPを手動保存</button></div><p class="ide190-muted">Package: <span data-role="package-id">未準備</span></p><pre data-role="reflection-result">まだ準備されていません。</pre></section>
    <section class="ide190-section"><strong>既存の手動反映ツール</strong><p class="ide190-muted">IDE-190は自動適用しません。ボタン操作で既存UIを開くだけです。</p><div class="ide190-row"><button data-action="paste-manager">ZIP貼付管理を開く</button><button data-action="diff-manager">Diff ZIP管理を開く</button></div></section>
    <section class="ide190-section"><div class="ide190-safe">Persistent Commit / GitHub自動反映 / Approval bypass / Repository直接変更は、このUIから実行できません。</div><div class="ide190-muted" data-role="message"></div></section>
  </div>
  <div class="ide190-footer"><button data-action="restore">表示を戻す</button><span class="ide190-muted">Display mode changes permission: false</span></div>
</div>`;
  ...
  async function prepareFromUI() {
    ...
    const filePaths = raw.split(/\r?\n|,/).map(function trim(x) { return x.trim(); }).filter(Boolean);
  ...
  function openDevelopmentAutomationConsole() {
  ...
  function getDevelopmentAutomationUIStatus() {
    return { status: "Ready", uiId: UI_ID, opened: Boolean(root() && root().style.display !== "none"), minimized: viewState.minimized, busy: viewState.busy, lastReflectionPackageId: viewState.lastReflectionPackageId, permissionChangesFromDisplayMode: false };
  }
