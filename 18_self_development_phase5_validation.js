...
(function (global) {
  ...
  async function runSelfDevelopment058Phase5Validation() {
    ...
    const syntheticClosure = namespace.evaluateSelfDevelopmentPhase5LiveEvidence({ transaction: { transactionId: "SYNTHETIC-NO-WRITE-CLOSURE-TEST", physicalWritePerformed: true, readbackVerified: true, rollbackVerified: true, repositoryRestored: true, acceptanceTokenConsumed: true, canonicalMutationPerformed: false } });
    check("Phase 5A live-evidence closure evaluator requires exact restored trial proof", syntheticClosure.complete === true && syntheticClosure.checks.physicalWritePerformed === true && syntheticClosure.checks.readbackVerified === true && syntheticClosure.checks.rollbackVerified === true && syntheticClosure.checks.repositoryRestored === true && syntheticClosure.checks.acceptanceTokenConsumed === true && syntheticClosure.checks.canonicalMutationPrevented === true, syntheticClosure, "Traceability");
    check("Phase 5A persisted live-evidence recovery API is available and separate from mutation authority", typeof namespace.recoverSelfDevelopmentPhase5LiveEvidence === "function" && typeof namespace.getSelfDevelopmentPhase5RecoveredLiveEvidence === "function", { recoveryApi: typeof namespace.recoverSelfDevelopmentPhase5LiveEvidence, getterApi: typeof namespace.getSelfDevelopmentPhase5RecoveredLiveEvidence, validationInvokedRecovery: false }, "Traceability");
    check("Decision 058 traceability supports pending or verified live-evidence state without false full completion", coverage.totalDecisionRequirements === 18 && coverage.phase5ScopeComplete === true && coverage.allDecisionRequirementsComplete === false && ((coverage.phase5LiveTrialComplete === true && coverage.liveTrialExecutionEvidenceRequiredForRequirementClosure === false) || (coverage.phase5LiveTrialComplete === false && coverage.liveTrialExecutionEvidenceRequiredForRequirementClosure === true)), coverage, "Traceability");
    check("Phase 5A traceability version matches candidate version", coverage.phase5Version === P5.version && P5.version === "0.5.7", { coverageVersion: coverage.phase5Version, candidateVersion: P5.version }, "Traceability");
    ...
    const report = Object.freeze({
      validationId: namespace.__internal.nextId("SELFDEV058-PHASE5-VALIDATION"), componentId: "SELF-DEVELOPMENT-058", decisionId: "EXTERNAL-010-DECISION-058", version: "0.5.7", phase: 5,
